...
To test our setup, we need to add a main function to the file ``glue/glue.cc``. The files ``bezier.cc`` and ``bezier.h`` can remain empty for now as they are not necessary to test our setup:
